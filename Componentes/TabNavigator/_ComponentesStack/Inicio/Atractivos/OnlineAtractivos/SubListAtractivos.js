...
export default class SubListAtractivos extends Component {
   ...
   ,{
    ...
    body: JSON.stringify({   
     ...
     //* Nota: Al pasarle al filtro select el 'IDDependencia', se le sumará uno, porque en este nivel
         //* el IDDependencia que vino de la lista principal es siempre uno, y se requiere que sea
         //* de dos para poder mostrar aquí el menú en nivel dos.
...
MegaMetodo (idMenu, nombre, Etiqueta, IDDependencia, sitio_idSitio, imgSitio, imgSitio2, imgSitio3, texto, titulo, descripcMapa, latitude, longitude) {
  ...
  if (this.state.connection_Status == 'Offline'){
   ...
   // const suma = IDDependencia + 1 // con esto ahora siempre va a ser tres y nunca va a ser dos. ASí es imposible
   if ( /* suma>2 */ sitio_idSitio != null){
    //OJO: IDDependencia siempreee será 2 aún tocando 'Festividades'
    ...
    this.props.navigation.navigate('AtractivoScreen',{
      ...
      this.props.navigation.navigate('SubListFinalAtractivos',{
        ...
        renderItem={({ item }) => (
         ...
         <TouchableOpacity
          ...
          onPress={this.MegaMetodo.bind(this,
               ...
               item.nombre,          // 
               item.Etiqueta,        // 
            //   item.SubEtiqueta,     // campo SubEtiqueta ya no existe en la bd
               item.IDDependencia,   // 
               item.sitio_idSitio,
               item.imgSitio,
               item.imgSitio2,
...
{ this.finActivityIndicator() //? para llamar al metodo que caduca el ActivityIndicator
         }
      {
        this.state.DefaultSpinner?
         ...
         :
         null //todo: se oculta este View cuando el 'DefaultSpinner' se vuelve false.
        }
